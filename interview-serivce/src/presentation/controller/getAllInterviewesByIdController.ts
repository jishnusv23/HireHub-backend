...
export const InterivewesByIdController = (dependancies: IDependancies) => {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const {id}=req.params
      const response=await IGeetAllInterivewesByIdUseCases(dependancies).execute(id)
      console.log("🚀 ~ file: getAllInterviewesByIdController.ts:12 ~ return ~ response:", response)
      if(!response){
